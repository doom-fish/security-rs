...
| Keychain | `SecAccessControlCreateWithFlags`, `SecAccessControlGetTypeID` | ✅ | Exposed through `AccessControl::create` / `AccessControl::type_id`; attached to items through `KeychainOptions::access_control`. The same `AccessControl` is taken by cryptokit-rs Secure Enclave key creation and apple-localauthentication's `LAContext::evaluate_access_control`, and `AccessControl::as_ptr` lends its `SecAccessControlRef`. |
